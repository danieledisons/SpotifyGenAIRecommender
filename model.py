...
class SpotifyRecommender:
    def __init__(self, data_path: str = "spotifydata.csv"):
    ...
    def recommend_tracks(self, track_name: str, n: int = 5) -> Dict[str, Union[List[Dict], str]]:
        """
        ...
        if track_name not in self.df["track_name"].values:
            return {
                "recommendations": [],
                "error": f"Track '{track_name}' not found in dataset"
            }
        ...
        for i in similarity_scores[1:n+1]:
            recommendations.append({
                ...
                "similarity_score": float(i[1])  # Convert numpy float to native float
            })
